...
export default class PhoneScreen extends Component {
    ...
    validatePhone() {
        ...
        const phoneValid = phone.match(/^(?=(?:[^\d]*[\d]){8})[\s\d#,.\-_+]*$/);
                ...
                <ImageBackground source={BG_IMAGE} style={styles.bgImage}>
                    <View style={styles.header}>
                        <Icon
                            ...
                            style={{padding:10}}
                        />
                        <Text style={styles.headerTitle}>{tran.t('phone_header')}</Text>
                        <Button
                            title={tran.t('save')}
                            type="clear"
                    ...
                    <View style={styles.itemList}>
                        <FormInput
                            label={tran.t('phoneNumber')}
                            refInput={input => (this.phoneInput = input)}
                            icon="user"
                            value={this.state.phone}
                            onChangeText={phone => this.setState({ phone })}
                            placeholder={tran.t('phoneNumber')}
                            errorMessage={
                                phoneValid ? null : tran.t('phoneValid')
                            }
                            onSubmitEditing={() => {
                                this.validatePhone();
                                this.updateData();
