...
export default class CreateFeedbackScreen extends Component {
    ...
    updateData() {
        ...
        if (titleValid && contentValid) {
            if(this.state.type===""){
                alert(tran.t('feedbackTypeValid'));
                return;
            }

            Axios.post(HOST_NAME+"feedback", {
                ...
                .then((response) => {
                    if (response.status === 200) {
                        ...
                        this.props.navigation.replace('FeedbackDetail',{ feedback: response.data.data.feedback, status:response.data.data.status.variables });
                    } else {
    ...
    render() {
        ...
        let feedback_type = this.state.feedback_type.map((value, index) => {
            return (
                ...
                <ImageBackground source={BG_IMAGE} style={styles.bgImage}>
                    <View style={styles.header}>
                        <Icon
                            ...
                            style={{padding:10}}
                        />
                        <Text style={styles.headerTitle}>{tran.t('create_feedback_header')}</Text>
                        <Button
                            title={tran.t('save')}
                            type="clear"
                    ...
                    <ScrollView>
                        <View style={styles.itemList}>
                            ...
                            />
                            <Text style={styles.inputPicker}>{"* "+tran.t('feedback_type')}</Text>
                            <Picker
                                selectedValue={this.state.type}
                                onValueChange={type => this.setState({ type: type })}
                            >
                                <Picker.Item label={tran.t('choose_feedback_type')} value="" />
                                {feedback_type}
                            </Picker>
                            <FormInput
    ...
    getData = async () => {
        await Axios.get(HOST_NAME+"feedback/type")
            .then((response) => {
                if (response.status === 200) {
                    this.setState({
                        feedback_type: response.data.data,
                    })
                }
            })
            .catch((error) => {
