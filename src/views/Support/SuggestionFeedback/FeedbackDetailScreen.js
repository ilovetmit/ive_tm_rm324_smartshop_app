...
export default class FeedbackDetailScreen extends Component {
    ...
    init() {
        this.state = {
            feedback: this.props.navigation.getParam("feedback"),
                ...
                <ImageBackground source={BG_IMAGE} style={styles.bgImage}>
                    ...
                    <ScrollView>
                        <View style={{paddingHorizontal:10,alignItems: 'center',justifyContent:'center'}}>
                            {(this.state.feedback.status==this.state.status['OPEN'])?
                                <Badge value={tran.t('support_status.open')} status="primary" />:
                                (this.state.feedback.status==this.state.status['COMPLETED'])?
                                    <Badge value={tran.t('support_status.completed')} status="success" />:
                                    (this.state.feedback.status==this.state.status['CLOSED'])?
                                        <Badge value={tran.t('support_status.closed')} status="warning" />:
                                        (this.state.feedback.status==this.state.status['CANCEL'])?
                                            <Badge value={tran.t('support_status.cancel')} status="error" />:
                                            <Badge value={tran.t('support_status.unknown')} status="error" />
                            }
                            <Badge value={this.state.feedback.feedback_type.type} status="primary" />
                            <Text style={styles.text}>{tran.t('title')} : {this.state.feedback.contents.title}</Text>
                            <Text style={styles.text}>{tran.t('content')} : {this.state.feedback.contents.body}</Text>
                            <Text style={styles.text}>{tran.t('created_at')} : {this.state.feedback.created_at}</Text>
                        </View>
                    </ScrollView>
                </ImageBackground>
            </View>
