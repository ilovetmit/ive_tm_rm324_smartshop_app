...
export default class InsuranceScreen extends Component {
    ...
    renderItem = (itemData, itemIdx, itemContainer) => {
        return (
            (itemData.insurance.End) ?
                ...
                >
                    <Image
                        source={{ uri: HOST_NAME + "/storage/in/" + itemData.insurance.image }}
                        style={styles.product_image}
                        PlaceholderContent={<ActivityIndicator />}
                        placeholderStyle={{ backgroundColor: '#FFF' }}
                    />
                    {/*<View style={styles.product_type}>*/}
                    {/*    <Text style={{color:'#FFFFFF',fontWeight: "bold"}}>{itemData.insurance.category}</Text>*/}
                    {/*</View>*/}
                    <Text style={styles.product_title} numberOfLines={1}>{itemData.insurance.name}</Text>
                    <Text style={styles.product_description} numberOfLines={2}>{itemData.insurance.description}</Text>
                    <View style={{ flexDirection: 'row', marginBottom: 5 }}>
                        <Text style={styles.product_price_type}>Monthly HKD </Text>
                        <Text style={styles.product_price}>{itemData.insurance.price.monthly}</Text>
                    </View>
                </TouchableOpacity>
        )
    };
        ...
        let teclastView = this.state.teclastView.map((value, index) => {
            return (
                ...
                >
                    ...
                    <View style={{ flexDirection: 'row', marginBottom: 5 }}>
                        <Text style={styles.product_price_type}>Monthly HKD </Text>
                        <Text style={styles.product_price}>{value.price.monthly}</Text>
                    </View>
                </TouchableOpacity>
            )
        });
    ...
    getAllData = async () => {
        Axios.get(HOST_NAME + HOST_API_VER + "insurance/all")
            .then((response) => {
                this.setState({
